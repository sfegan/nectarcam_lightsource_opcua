...
def unwrap_variants(fn):
    ...
    python-opcua passes arguments as ``ua.Variant`` objects rather than plain
    Python values.  This decorator unwraps every argument (after ``parent``)
    before forwarding to the real implementation, so none of the method bodies
    need to call ``.Value`` themselves.
    """
    @functools.wraps(fn)
    def wrapper(self, parent, *args):
        ...
        return fn(self, parent, *unwrapped)
    return wrapper
...
_AIVFF_STATUS_MASK = bytes.fromhex("202020404040406060606F737F606060606080808" "0A0A0A5C0C0C00D0A")
...
class CalBoxConfig:
    ...
    def __init__(self, product: int, release: int):
        ...
        # Resize template to n bytes (pad with 0 if needed)
    ...
    def to_cmd(self) -> bytes:
        ...
        c = bytearray(n)
        # C0-C4 <- S0-S4  (LED mask + voltage set)
        c[0:5] = self.s[0:5]
        # C5-C15 <- S7-S17  (duration, frequency, divider, width)
        c[5:16] = self.s[7:18]
        # C16 <- S22  (control mask)
        c[16] = self.s[22]
        # C17-C20 <- S23-S26  (central current, SPE only)
        if self.product == ProductCode.SPE:
            c[17:21] = self.s[23:27]
        # Product code byte, then CRLF
        c[-3] = self.s[-6]
        c[-2] = 0x0D
        c[-1] = 0x0A
        return bytes(c)
...
class CalBoxHardware:
    ...
    All socket I/O uses a configurable timeout (default 2 s).
    The lock ensures commands are serialised when called from
    multiple threads (OPC UA method callbacks + poll thread).
    """
    ...
    def is_connected(self) -> bool:
        return self._sock is not None and self._link_ok
    ...
    def ensure_connected(self) -> bool:
        ...
        self.close()
        return self.connect() and self.authenticate()
    ...
    def _recv_until_crlf(self) -> bytes:
        """Read bytes until \\r\\n; return payload excluding the delimiter."""
        buf = b""
        while True:
    ...
    # ----------------------------------------------------------
    # Simple 4-byte command frame
    # ----------------------------------------------------------
    def _build_simple_cmd(self, code: int) -> bytes:
        """
        Layout: [cmd_code, product_code, 0x0D, 0x0A]
        Matches C++: code | (product<<8) | (0x0A0D<<16), little-endian.
        """
    ...
    def _wait_for_reply(self) -> CalBoxConfig:
        """Read a CRLF-terminated status frame and decode it."""
        raw = self._recv_until_crlf()
        # Re-append CRLF: C++ code adds them back after stripping
        raw_with_crlf = raw + b"\r\n"
    ...
    # used by all single-parameter Set* commands
    # ----------------------------------------------------------
    def _get_and_modify(self, modify_fn) -> CalBoxConfig:
        """
        1. GET_STATUS to read current config
        2. apply modify_fn(cfg)
        3. send resulting Configure frame
        Returns the new status CalBoxConfig.
        """
    ...
    # ----------------------------------------------------------
    # High-level commands  (all thread-safe via lock)
    # ----------------------------------------------------------
    def get_status(self) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._exec_simple_cmd(Cmd.GET_STATUS)

    def start(self) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
    ...
    def set_leds(self, mask: int) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(lambda c: c.set_leds(mask))

    def set_voltage(self, volts: float) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(lambda c: c.set_voltage(volts))

    def set_duration(self, duration: int) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(lambda c: c.set_duration(duration))

    def set_frequency(self, dividend: float, divider: int) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            def _mod(c):
            ...
            return self._get_and_modify(_mod)

    def set_central_current(self, mA: float) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(lambda c: c.set_central_current(mA))

    def set_width(self, width: int) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(lambda c: c.set_width(width))

    def set_trigger_control(self, lemo: bool, fiber1: bool, fiber2: bool,
                            external: bool, enabled: bool) -> CalBoxConfig:
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            return self._get_and_modify(
    ...
    def configure(self, led_mask: int, voltage_set: float, duration: int,
        ...
        with self._lock:
            if not self.ensure_connected():
                raise OSError("Not connected")
            # Start from last known state so unspecified fields keep their value
            cfg = CalBoxConfig(self.product_code, self.release)
            if self._last_cfg:
            ...
            cfg.set_width(width)
            cfg.set_control(lemo_out, fiber1_out, fiber2_out,
                            external, enabled)
...
class CalibrationBoxServer:
    """
    OPC UA server that wraps CalBoxHardware and exposes the full
    ...
    def __init__(
        ...
        if self.opcua_users:
            self.server.user_manager.set_user_manager(self._user_manager)

        self.idx     = self.server.register_namespace(
            "http://nectarcam.calibrationbox"
        )
        objects      = self.server.get_objects_node()
        self.root    = objects.add_object(self.idx, device_root_name)
    ...
    def _user_manager(self, isession, username, password):
        return self.opcua_users.get(username) == password

    # ----------------------------------------------------------
    # Monitoring nodes
    # ----------------------------------------------------------
    def _add_var(self, name: str, default, vtype):
        """Create Monitoring/<name>/<name>_v node."""
        container = self.monitoring.add_object(self.idx, name)
        var = container.add_variable(
    ...
    def _create_monitoring(self):
        ...
        self._add_var("led_mask",           8191,    ua.VariantType.Int32)
        self._add_var("voltage_set",        10.0,    ua.VariantType.Float)
        self._add_var("voltage_actual",     10.0,    ua.VariantType.Float)
        self._add_var("duration",           0,       ua.VariantType.Int32)
        self._add_var("frequency_dividend", 10000.0, ua.VariantType.Float)
        self._add_var("frequency_divider",  1,       ua.VariantType.Int32)
        self._add_var("width",              1,       ua.VariantType.Int32)
    ...
    def _create_methods(self):
        def m(name, cb, in_types):
            self.device.add_method(self.idx, name, cb, in_types, [])

        m("Start",     self._m_start,     [])
        m("Stop",      self._m_stop,      [])
        m("Reboot",    self._m_reboot,    [])
        m("Reconnect", self._m_reconnect, [])
        m("GetStatus", self._m_get_status,[])

        m("SetLeds",     self._m_set_leds,     [ua.VariantType.Int32])
        m("SetVoltage",  self._m_set_voltage,  [ua.VariantType.Float])
        m("SetDuration", self._m_set_duration, [ua.VariantType.Int32])
        m("SetCurrent",  self._m_set_current,  [ua.VariantType.Float])
        m("SetWidth",    self._m_set_width,    [ua.VariantType.Int32])

        m("SetFrequency", self._m_set_frequency,
    ...
    @unwrap_variants
    def _m_start(self, parent):
        return self._dispatch(self.hardware.start)

    @unwrap_variants
    def _m_stop(self, parent):
        return self._dispatch(self.hardware.stop)

    @unwrap_variants
    def _m_reboot(self, parent):
        try:
    ...
    def _m_set_leds(self, parent, mask: int):
        return self._dispatch(lambda: self.hardware.set_leds(mask))

    @unwrap_variants
    def _m_set_voltage(self, parent, volts: float):
        return self._dispatch(lambda: self.hardware.set_voltage(volts))

    @unwrap_variants
    def _m_set_duration(self, parent, duration: int):
        return self._dispatch(lambda: self.hardware.set_duration(duration))

    @unwrap_variants
    def _m_set_frequency(self, parent, dividend: float, divider: int):
        return self._dispatch(
            lambda: self.hardware.set_frequency(dividend, divider)
        )

    @unwrap_variants
    def _m_set_current(self, parent, mA: float):
        return self._dispatch(lambda: self.hardware.set_central_current(mA))

    @unwrap_variants
    def _m_set_width(self, parent, width: int):
    ...
    def _apply_config(self, cfg: CalBoxConfig):
        ...
        # Derive FSM state from hardware report
        if cfg.error_mask():
